...
-- ─────────────────────────────────────────────────────────────────────────────
-- 1. migrate_tables_to_new (Utilizes encapsulated logger module)
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_us_new.migrate_tables_to_new(
...
BEGIN
  ...
  FOR tbl IN (
    ...
    EXCEPTION WHEN ERROR THEN
      CALL unravel_share_us_new._log_error(current_run_ts, tbl.table_id, @@project_id, 'col_list resolution failed: '||@@error.message, exec_sql); CONTINUE;
    END;
    IF col_list IS NULL THEN
      CALL unravel_share_us_new._log_error(current_run_ts, tbl.table_id, @@project_id, 'col_list is NULL', NULL); CONTINUE;
    END IF;
    ...
    BEGIN EXECUTE IMMEDIATE exec_sql;
    EXCEPTION WHEN ERROR THEN
      CALL unravel_share_us_new._log_error(current_run_ts, tbl.table_id, @@project_id, 'DDL failed: '||@@error.message, exec_sql); CONTINUE;
    END;
    ...
    BEGIN EXECUTE IMMEDIATE exec_sql;
    EXCEPTION WHEN ERROR THEN
      CALL unravel_share_us_new._log_error(current_run_ts, tbl.table_id, @@project_id, 'Data copy failed: '||@@error.message, exec_sql);
    END;
  END FOR;
END;
