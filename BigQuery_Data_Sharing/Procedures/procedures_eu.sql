...
CREATE SCHEMA IF NOT EXISTS unravel_share_EU_new
  OPTIONS (location = 'EU');

CREATE SCHEMA IF NOT EXISTS unravel_share_EU_new_projects_list
  OPTIONS (location = 'EU');
...
ALTER TABLE `unravel_share_EU_projects_list.projects_table`
...
TRUNCATE TABLE `unravel_share_EU_projects_list.projects_table`;


-- ─────────────────────────────────────────────────────────────────────────────
-- UTILITY A: _log_error (Modularized Diagnostic Logger)
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new._log_error(
  run_ts TIMESTAMP, dest_table STRING, project_id STRING, error_message STRING, failed_sql STRING
)
BEGIN
  INSERT INTO `unravel_share_EU_new.error_log` (run_ts, dest_table, project_id, error_message, failed_sql, logged_at)
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new._update_project_ledger(
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new_projects_list.create_projects_table(
  dataset_name STRING, 
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new.export_billing_data_incremental(
  dataset_name STRING, 
...
BEGIN
  ...
  EXCEPTION WHEN ERROR THEN
    CALL unravel_share_EU_new._log_error(current_run_ts, dest_table, billing_export_project, 'Billing DDL failed: '||@@error.message, exec_sql); RETURN;
  END;
  ...
  EXCEPTION WHEN ERROR THEN
    CALL unravel_share_EU_new._log_error(current_run_ts, dest_table, billing_export_project, 'Billing col_list failed: '||@@error.message, exec_sql); RETURN;
  END;
  
  IF billing_col_list IS NULL THEN
    CALL unravel_share_EU_new._log_error(current_run_ts, dest_table, billing_export_project, 'billing_col_list resolves to NULL', NULL); RETURN;
  END IF;

  CALL unravel_share_EU_new._build_billing_typed_select(
  ...
  BEGIN EXECUTE IMMEDIATE exec_sql;
  EXCEPTION WHEN ERROR THEN
    CALL unravel_share_EU_new._log_error(current_run_ts, dest_table, billing_export_project, 'Billing incremental insert failed: '||@@error.message, exec_sql);
  END;
END;
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new._build_typed_select(
  dataset_name     STRING,
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new._build_billing_typed_select(
  dataset_name     STRING,
  dest_table_name  STRING,
  region           STRING,
  col_list         STRING,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. export_metadata_incremental_EU (Utilizes project fallback loops + modular ledger evaluating)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  FOR table_row IN (SELECT * FROM UNNEST(tables)) DO
    ...
    IF cfg.is_by_org THEN
      ...
      EXCEPTION WHEN ERROR THEN
        CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, 'BY_ORG', 'Org DDL layout schema configuration collapsed: '||@@error.message, exec_sql);
      END;
    ELSE
      ...
      WHILE ddl_project_idx < total_ddl_projects AND NOT ddl_success DO
        ...
        EXCEPTION WHEN ERROR THEN
          ...
          CALL unravel_share_EU_new._update_project_ledger(control_ledger, @@error.message, test_project, current_table);
          CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, test_project, 'DDL trial block exception intercept: '||@@error.message, exec_sql);
        END;
      END WHILE;
    END IF;

    IF NOT ddl_success OR baseline_project IS NULL THEN
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, 'GLOBAL_FALLBACK', 'All candidate trace components threw DDL errors. Table iteration terminated.', NULL);
      CONTINUE;
    END IF;
    ...
    BEGIN 
      EXECUTE IMMEDIATE exec_sql;
    EXCEPTION WHEN ERROR THEN
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, baseline_project, 'col_list matrix dynamic table build collapsed: '||@@error.message, exec_sql);
      CONTINUE;
    END;
    ...
    EXCEPTION WHEN ERROR THEN
      ...
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, baseline_project, 'col_list metric string aggregated output trace collapse: '||@@error.message, exec_sql);
      CONTINUE;
    END;
    ...
    IF col_list IS NULL THEN
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, baseline_project, 'col_list aggregated resolution mapped exclusively to NULL components.', NULL);
      CONTINUE;
    END IF;

    CALL unravel_share_EU_new._build_typed_select(dataset_name, dest_table_name, region, col_list, typed_select);
    ...
    IF cfg.is_by_org THEN
      IF cfg.strategy = 'SNAPSHOT_MERGE' THEN
        ...
        CALL unravel_share_EU_new._flush_batch(dataset_name,dest_table_name,col_list,region,
      ...
      ELSE
        ...
        BEGIN EXECUTE IMMEDIATE exec_sql;
        EXCEPTION WHEN ERROR THEN
          CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, 'BY_ORG', 'BY_ORG clear operations exception hit: '||@@error.message, exec_sql);
          CONTINUE;
        END;
      ...
      BEGIN EXECUTE IMMEDIATE exec_sql;
      EXCEPTION WHEN ERROR THEN
        CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, 'BY_ORG', 'BY_ORG batch insertion operation collapsed: '||@@error.message, exec_sql);
      END;
      CONTINUE;
    END IF;
    ...
    FOR project_row IN (SELECT * FROM UNNEST(project_ids)) DO
      ...
      IF project_id IS NULL OR project_id = '' THEN
        CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, project_id, 'project_id value validated as NULL or clean empty string context.', NULL);
        CONTINUE;
      END IF;
...
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE PROCEDURE unravel_share_EU_new._flush_batch(
...
BEGIN
  ...
  IF cfg.strategy IN ('INCREMENTAL_APPEND','AUDIT_APPEND') THEN
    ...
    EXCEPTION WHEN ERROR THEN
      ...
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, source_scope, 'Macro batch append failed. Unnesting loop: '||@@error.message, exec_sql);
      ...
      ELSE
        FOR idx IN (SELECT p FROM UNNEST(batch_projects) AS p) DO
          ...
          EXCEPTION WHEN ERROR THEN
            ...
            CALL unravel_share_EU_new._update_project_ledger(control_ledger, @@error.message, fallback_project, cfg.table_name);
            CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, fallback_project, 'Isolated fallback copy failed: '||@@error.message, fallback_sql);
          END;
        END FOR;
      END IF;
    END;
  ...
  ELSEIF cfg.strategy = 'SNAPSHOT_MERGE' AND cfg.is_by_org THEN
    ...
    BEGIN 
      EXECUTE IMMEDIATE exec_sql;
    EXCEPTION WHEN ERROR THEN
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, source_scope, 'BY_ORG snapshot merge failed: '||@@error.message, exec_sql);
    END;
  ...
  ELSEIF cfg.strategy = 'SNAPSHOT_MERGE' AND NOT cfg.is_by_org THEN
    ...
    EXCEPTION WHEN ERROR THEN
      -- Log macro batch error via centralized module anyway
      CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, source_scope, 'Batch merge identity conflict hit. Initiating loops fallback: '||@@error.message, exec_sql);
      ...
      FOR idx IN (SELECT p FROM UNNEST(batch_projects) AS p) DO
        ...
        EXCEPTION WHEN ERROR THEN
          -- Route checks and writes securely through modularized logic paths
          CALL unravel_share_EU_new._update_project_ledger(control_ledger, @@error.message, fallback_project, cfg.table_name);
          CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, fallback_project, 'Isolated fallback merge collapsed: '||@@error.message, fallback_sql);
        END;
      END FOR;
    END;
    
  ELSE
    CALL unravel_share_EU_new._log_error(current_run_ts, dest_table_name, source_scope, 'Unknown strategy context detected: '||cfg.strategy, NULL);
  END IF;
END;
...
BEGIN
  ...
  FOR t_row IN (SELECT * FROM UNNEST(tables)) DO
    ...
    IF project_ids IS NULL OR ARRAY_LENGTH(project_ids) = 0 THEN
      CALL unravel_share_EU_new._log_error(CURRENT_TIMESTAMP(), current_loop_table, 'WRAPPER', 'Sequence tracking skipped. Zero allowed entries tracked inside control schema.', ledger_lookup_sql);
      CONTINUE;
    END IF;
